...
def resize_filesystem(pod_id: int, volume_id: str, old_size_gb: int, new_size_gb: int):
    ...
    with Session(engine) as db:
        try:
            ...
            # Update the storage expansion history to mark as completed
            try:
                import json
                ...
                # Find the most recent expansion event and mark it as completed
                for event in reversed(history):
                    if event.get("status") == "initiated" and event.get("volume_id") == volume_id:
                        event["status"] = "completed"
                        event["message"] = f"Storage expansion completed: {old_size_gb}GB → {new_size_gb}GB (Filesystem resized)"
                ...
                db.add(pod)
                db.commit()
                logger.info(f"Updated storage expansion history for pod {pod_id}")
                
            except Exception as e:
                logger.warning(f"Failed to update storage expansion history for pod {pod_id}: {e}")
            
        except Exception as e:
            logger.error(f"[resize_filesystem] Failed to resize filesystem for pod {pod_id}: {e}")
            
            # Log the failure in storage expansion history
            try:
                import json
                from datetime import datetime
                existing_history = pod.storage_expansion_history or "[]"
                ...
                # Find the most recent expansion event and mark it as failed
                for event in reversed(history):
                    if event.get("status") == "initiated" and event.get("volume_id") == volume_id:
                        event["status"] = "failed"
                        event["message"] = f"Storage expansion failed: {old_size_gb}GB → {new_size_gb}GB (Filesystem resize error: {str(e)})"
                        event["failed_at"] = datetime.now().isoformat()
                        event["error"] = str(e)
                        break
                ...
                logger.warning(f"Failed to update storage expansion history for pod {pod_id}: {history_error}")
